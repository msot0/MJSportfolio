...
// Music is disabled when this game is embedded inline on the portfolio page
// (loaded with ?embed=1) so it doesn't play in the background while browsing
// the rest of the site. Opening "Play Fullscreen" loads this page without
// that param, so the real game keeps its full audio experience.
const _isEmbedded = new URLSearchParams(location.search).get('embed') === '1';
...
function _playNow(previewUrl, entry) {
  ...
  if (_isEmbedded) {
    // No audio in the inline portfolio embed - just track state so the
    // rest of the game's UI (song notification, etc.) still works normally.
