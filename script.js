...
function showSection(id, el) {
  ...
  // Embedded games (Duel Dash, Radio Rift) use data-src instead of src so they
  // don't load - or make any sound - until their tab is actually open.
  // Leaving a tab fully unloads its iframe (src = "") so any playing audio
  // stops immediately instead of continuing in the background.
  document.querySelectorAll(".content iframe[data-src]").forEach(frame => {
    if (target.contains(frame)) {
      if (!frame.getAttribute("src")) {
        frame.src = frame.dataset.src;
        frame.addEventListener("load", resizeGameEmbeds, { once: true });
      }
...
// A ResizeObserver is the reliable fix here: it fires the instant a
// game-embed-wrap actually changes size, including the moment a hidden
// tab (display:none, width 0) becomes visible again - which is exactly
// the case the old resize/load-only approach could miss and leave
// un-scaled, full-size, and clipped by the card's rounded corners.
